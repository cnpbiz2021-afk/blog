...
const SLUG_MAP_PATH = path.join(ROOT, "data", "slug-map.json");

/* ==================== 0. 슬러그 매핑(post.id -> slug) 영속화 ====================
 * 한 번 생성된 슬러그는 이 파일에 저장해두고 이후 실행에서는 그대로 재사용한다.
 * -> 이미 검색엔진에 색인된 글의 URL이 나중에 바뀌어 404가 나는 것을 방지.
 * -> 새 슬러그 생성 규칙(길이 제한 등)이 바뀌어도 기존 글에는 영향 없음.
 */
// slug-map.json이 아직 없는 최초 1회 실행 시, posts/ 폴더에 이미 생성돼 있는
// 파일명(shortId 기준)을 그대로 읽어와 매핑을 채운다.
// -> 이 마이그레이션이 없으면 첫 실행에서 기존 글의 슬러그가 새 길이 제한 규칙으로
//    재계산되어 이미 색인된 URL이 바뀌어 버린다.
async function bootstrapSlugMapFromExistingFiles(posts) {
  ...
  for (const post of posts) {
    ...
    const shortId = key.replace(/-/g, "").slice(0, 8);
    if (!key || !shortId) continue;
    const match = htmlFiles.find(f => f.endsWith(`-${shortId}.html`));
    if (match) map[key] = match.slice(0, -".html".length);
  }
  return map;
}

async function loadSlugMap(posts) {
  try {
    const text = await fs.readFile(SLUG_MAP_PATH, "utf-8");
    return JSON.parse(text);
  } catch {
    console.log("ℹ️ slug-map.json이 없어 기존 posts/ 파일명에서 슬러그를 마이그레이션합니다.");
    return bootstrapSlugMapFromExistingFiles(posts);
  }
}

async function saveSlugMap(slugMap) {
  await fs.mkdir(path.dirname(SLUG_MAP_PATH), { recursive: true });
...
// 슬러그 최대 길이(하이픈 기준 단어 단위로 자름) — 파일시스템/URL 길이 문제 방지
const SLUG_MAX_LENGTH = 50;

function truncateSlug(base, maxLength = SLUG_MAX_LENGTH) {
  if (base.length <= maxLength) return base;
  const cut = base.slice(0, maxLength);
  const lastHyphen = cut.lastIndexOf("-");
  // 잘린 위치가 너무 앞쪽이 아니라면 단어 경계(하이픈)에서 자르고, 아니면 그냥 자름
  return lastHyphen > maxLength * 0.5 ? cut.slice(0, lastHyphen) : cut;
}

function computeSlug(post) {
  const base = truncateSlug(slugify(post.title) || "post");
  const shortId = String(post.id || "").replace(/-/g, "").slice(0, 8) || "0000";
  return `${base}-${shortId}`;
}

// slugMap: { [post.id]: slug } — 이미 기록된 글은 항상 저장된 슬러그를 그대로 반환한다.
function getPostSlug(post, slugMap = {}) {
  const key = String(post.id || "");
  if (key && slugMap[key]) return slugMap[key];
  const slug = computeSlug(post);
...
function getRelatedPosts(post, allPosts, slugMap, count = 3) {
  const currentTokens = new Set(extractTitleTokens(post.title));
  const currentSlug = getPostSlug(post, slugMap);

  const scored = allPosts
    .filter(p => getPostSlug(p, slugMap) !== currentSlug)
    .map(p => {
...
function renderRelatedPostsBlock(post, allPosts, slugMap) {
  const related = getRelatedPosts(post, allPosts, slugMap, 3);
  if (related.length === 0) return "";

  const items = related
    .map(p => {
      const href = `${encodeURIComponent(getPostSlug(p, slugMap))}.html`;
...
async function renderPostPage(template, post, allPosts, slugMap) {
  const slug = getPostSlug(post, slugMap);
  ...
  const relatedPostsBlock = renderRelatedPostsBlock(post, allPosts, slugMap);
...
function renderListItemHtml(post, slugMap) {
  const slug = getPostSlug(post, slugMap);
...
async function injectListIntoIndex(posts, slugMap) {
  ...
  const listHtml = posts.map(post => renderListItemHtml(post, slugMap)).join("\n          ");
...
async function regenerateSitemap(posts, slugMap) {
  const today = new Date().toISOString().split("T")[0];
  const urls = [
    { loc: `${SITE_ORIGIN}/`, lastmod: today, changefreq: "weekly", priority: "1.0" },
    ...posts.map(post => ({
      loc: `${SITE_ORIGIN}/posts/${encodeURIComponent(getPostSlug(post, slugMap))}.html`,
...
async function main() {
  ...
  const slugMap = await loadSlugMap(posts);

  await fs.mkdir(path.join(ROOT, "posts"), { recursive: true });

  for (const post of posts) {
    const html = await renderPostPage(template, post, posts, slugMap);
    const slug = getPostSlug(post, slugMap);
  ...
  await injectListIntoIndex(posts, slugMap);
  await regenerateSitemap(posts, slugMap);
  await saveSlugMap(slugMap);
